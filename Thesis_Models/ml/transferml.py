...
BHP = 145 # 145 or 150
RUN_NAME = f"transfer-{DATA_LOAD}-{BHP}"
...
def main(num_folds=5,model_path="final/"):
    ...
    # Tendeka training
    training_dl = utils.getDataLoader(f"../fitting/model_data/{BHP}-{DATA_LOAD}-RandTrainingData.csv")
    validate_dl = utils.getDataLoader(f"../fitting/model_data/{BHP}-{DATA_LOAD}-RandValidationData.csv", shuffle=False)

    # Train the model
    logger = pl.loggers.TensorBoardLogger(osp.join(model_path, "lightning_logs", RUN_NAME + "SimulatedData"))
    trainer = pl.Trainer(
            max_epochs=1000,
    ...
    # Train the model
    logger = pl.loggers.TensorBoardLogger(osp.join(model_path, "lightning_logs", RUN_NAME + "realData"))
    trainer = pl.Trainer(
            max_epochs=1000,
            default_root_dir = model_path,
            logger = logger,
